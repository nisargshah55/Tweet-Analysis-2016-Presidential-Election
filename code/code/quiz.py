...
tweets_negative = []
for line in open('train_nega_tweets_2016.txt').readlines():
    ...
    items=[0,temp]
    tweets_negative.append(items)
  
tweets_positive = []
for line in open('train_posi_tweets_2016.txt').readlines():
    tweet=json.loads(line)
    
    pos=tweet['text']
    items=[1,pos]
    tweets_positive.append(items)

  
# Extract the vocabulary of keywords
vocab = dict()
for label,text in tweets_negative:
    for term in text.split():
        term = term.lower()
        if len(term) > 2 and term not in words:
...
X = []
y = []
for class_label, tweet_text in tweets_negative:
    x = [0] * len(vocab)
...
for class_label, tweet_text in tweets_positive:
    x = [0] * len(vocab)
...
for idx, [tweet_id, tweet_text] in enumerate(tweets1):
    if y[idx]==1:
        dict().update({tweet_id:'true'})
    else:
        dict().update({tweet_id:'false'})
